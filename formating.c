...
void formating_monomial(char x_string[], double coef, bool is_print_plus) //Форматирует одночлен. Если при нем множитель 1 или -1, то убирает его. 
...
void formating_equation(double a, double b, double c)//Комбинирует отформартированные одночлены и возвращает полное выражение
{
	...
	formating_monomial("x", b, a != 0 && b > 0); //Форматирование одночлена с х. Если a=0, то след по старшинству члену не печатать слева '+'
	if (!(isnul(c)))
	{
